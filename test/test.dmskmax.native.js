...
var dmskmax = tryRequire( resolve( __dirname, './../lib/dmskmax.native.js' ) );
var opts = {
...
tape( 'the function has an arity of 5', opts, function test( t ) {
	t.strictEqual( dmskmax.length, 5, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the maximum value of a strided array according to a mask', opts, function test( t ) {
	var mask;
	var x;
	var v;
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = dmskmax( 0, x, 1, mask, 1 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = dmskmax( -1, x, 1, mask, 1 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
